...
Переменные окружения:
  POST_ID                    — id поста для публикации (обязательно)
  TELEGRAM_BOT_TOKEN
  TELEGRAM_CHANNEL_ID         — например "@allindsgn"
  THREADS_ACCESS_TOKEN        — long-lived токен (опционально, если Threads ещё не настроен)
  THREADS_USER_ID             — ID аккаунта Threads (опционально)
  PUBLIC_ASSETS_BASE_URL      — публичный URL до assets/photos, нужен для фото в Threads
                                 например: https://raw.githubusercontent.com/USER/REPO/main/assets/photos
"""
import json
import os
import time
...
def publish_threads(post, photo_filename):
    ...
    time.sleep(5)  # Threads рекомендует паузу перед публикацией контейнера
